			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
					...
					if (@$guardar) {
						...
						$insti = $_POST["insti"];
						$resultado=funciones::modificarUsuario($ced,$cedu,$nomb,$apel,$pass,$rol,$est,$insti);
						if ($resultado == 0) {
							$op = 3;
						}
						else
					...
					if (@$buscar) {
						$ced = $_POST["ced"];
						$resultado=funciones::buscarUsuario($ced);
						if($resultado == 0)
						{
							$op=1;
						}
						else
						{
							...
							$apel = $resultado['APELLIDO'];
							$pass = $resultado['PASSWORD'];
							$rol = $resultado['ROL'];
							$est = $resultado['ESTADO'];
							$insti = $resultado['ID_INSTIT'];
							$op=2;
						}
					}
					?>
						<div class="row">
							<div class="col-lg-12">
								<section class="panel">'
									...
									<form class="form-horizontal form-bordered" method="post" action="modificar-usuario.php">
										<div class="panel-body">
											<div class="form-group">
												<label class="col-md-4 control-label" for="inputPlaceholder">Cédula</label>
												<div class="col-md-3">
													<div class="input-group">
														<?php
														$resultado=funciones::consultarUsuario();
															echo '<select class="form-control mb-md-6" name = "ced">';
															foreach ($resultado as $row) {
																if(@$ced == $row['CEDULA'])
																{
																	echo '<option selected>'.$row['CEDULA'].'</option>';
																}
																else
																{
															...
															echo '</select>';
														echo '<input type="hidden" class="form-control" id="inputDefault" name="cedu" value='.@$ced.'>';
														?>
														<span class="input-group-btn">
																<button class="btn btn-default" type="submit" name="buscar"  id="buscar" value=1><i class="fa fa-search"></i></button>
															</span>
													</div>
												</div>
											</div>
											<div class="form-group">
												...
												<div class="col-md-3">
												<?php
													echo '<input type="text" class="form-control" id="inputDefault" name="nomb" value=\''.@$nomb.'\'>';
												?>
												</div>
											</div>
											<div class="form-group">
												<label class="col-md-4 control-label" for="inputDefault">Apellido</label>
												<div class="col-md-3">
												<?php
													echo '<input type="text" class="form-control" id="inputLastName" name="apel" value=\''.@$apel.'\'>';
													?>
												</div>
											</div>
											<div class="form-group">
												<label class="col-md-4 control-label" for="inputPlaceholder">Contraseña</label>
												<div class="col-md-3">
												<?php
													echo '<input type="text" class="form-control" placeholder="" id="inputPassword" name="pass" value='.@$pass.'>';
													?>
												</div>
											</div>
											<div class="form-group">
												<label class="col-md-4 control-label" for="inputSuccess">Rol</label>
												<div class="col-md-3">
													<select class="form-control mb-md" name="rol">
													<?php
													if (!@$rol) {
														...
														echo '<option>Administrador</option>';
														echo '<option selected>Investigador</option>';
														echo '<option>Profesor</option>';
														echo '<option>Estudiante</option>';
													}elseif (@$rol == "Profesor") {
														echo '<option>Administrador</option>';
														echo '<option>Investigador</option>';
														echo '<optionselected>Profesor</option>';
														echo '<option>Estudiante</option>';
													}elseif (@$rol == "Estudiante") {
														echo '<option>Administrador</option>';
														echo '<option>Investigador</option>';
														echo '<option>Profesor</option>';
														echo '<option selected>Estudiante</option>';
													}
													?>
													</select>
												</div>
											...
											<div class="form-group">
												<label class="col-md-4 control-label" for="inputDefault">Código de la institución</label>
												<div class="col-md-3">
												<?php
													echo '<input type="text" class="form-control" id="inputDefault" name="insti" value='.@$insti.'>';
													?>
												</div>
											</div>
											<div class="form-group">
